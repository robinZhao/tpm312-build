...
{
	snd_soc_component_write(codec, ES8316_RESET_REG00, 0x3F);
	usleep_range(5000, 5500);
	return snd_soc_component_write(codec, ES8316_RESET_REG00, 0x03);
}
			      ...
			      unsigned int fmt)
{
	...
	iface    = snd_soc_component_read(codec, ES8316_IFACE);
	adciface = snd_soc_component_read(codec, ES8316_ADC_IFACE);
	daciface = snd_soc_component_read(codec, ES8316_DAC_IFACE);
	...
	snd_soc_component_write(codec, ES8316_IFACE, iface);
	snd_soc_component_write(codec, ES8316_ADC_IFACE, adciface);
	snd_soc_component_write(codec, ES8316_DAC_IFACE, daciface);
	return 0;
}
			      ...
			      struct snd_soc_dai *dai)
{
	...
	snd_soc_component_write(codec, ES8316_RESET_REG00, 0xC0);
	snd_soc_component_write(codec, ES8316_SYS_PDN_REG0D, 0x00);
	...
	if (playback) {
		...
		snd_soc_component_write(codec, ES8316_SYS_LP2_REG0F, 0x1F);
		snd_soc_component_write(codec, ES8316_HPMIX_SWITCH_REG14, 0x88);
		snd_soc_component_write(codec, ES8316_HPMIX_PDN_REG15, 0x00);
		snd_soc_component_write(codec, ES8316_HPMIX_VOL_REG16, 0xBB);
		snd_soc_component_write(codec, ES8316_CPHP_PDN2_REG1A, 0x10);
		snd_soc_component_write(codec, ES8316_CPHP_LDOCTL_REG1B, 0x30);
		snd_soc_component_write(codec, ES8316_CPHP_PDN1_REG19, 0x02);
		snd_soc_component_write(codec, ES8316_DAC_PDN_REG2F, 0x00);
		snd_soc_component_write(codec, ES8316_CPHP_OUTEN_REG17, 0x66);
		snd_soc_component_update_bits(codec, ES8316_CLKMGR_CLKSW_REG01,
				    ES8316_CLKMGR_DAC_MCLK_MASK |
		...
		snd_soc_component_update_bits(codec, ES8316_GPIO_SEL_REG4D,0x02,0x2);
				...
				struct snd_soc_dai *dai)
{
	...
	if (playback) {
		snd_soc_component_write(codec, ES8316_CPHP_OUTEN_REG17, 0x00);
		snd_soc_component_write(codec, ES8316_DAC_PDN_REG2F, 0x11);
		snd_soc_component_write(codec, ES8316_CPHP_LDOCTL_REG1B, 0x03);
		snd_soc_component_write(codec, ES8316_CPHP_PDN2_REG1A, 0x22);
		snd_soc_component_write(codec, ES8316_CPHP_PDN1_REG19, 0x06);
		...
		snd_soc_component_update_bits(codec, ES8316_GPIO_SEL_REG4D,0x02,0x0);
				...
				struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		snd_soc_component_update_bits(codec, ES8316_SDP_DACFMT_REG0B,
				    ES8316_DACWL_MASK, val);
	else
		snd_soc_component_update_bits(codec, ES8316_SDP_ADCFMT_REG0A,
...
static int es8316_mute(struct snd_soc_dai *dai, int mute,int direction)
{
	...
	if (mute) {
		...
		msleep(100);
		snd_soc_component_write(codec, ES8316_DAC_SET1_REG30, 0x20);
	} else {
		snd_soc_component_write(codec, ES8316_DAC_SET1_REG30, 0x00);
		msleep(130);
		if (!es8316->hp_inserted)
				 ...
				 enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_OFF:
		...
		snd_soc_component_write(codec, ES8316_SYS_LP1_REG0E, 0x3F);
		snd_soc_component_write(codec, ES8316_SYS_LP2_REG0F, 0x1F);
		snd_soc_component_write(codec, ES8316_RESET_REG00, 0x00);
		break;
	}
...
static int es8316_init_regs(struct snd_soc_component *codec)
{
	...
	usleep_range(5000, 5500);
	snd_soc_component_write(codec, ES8316_RESET_REG00, 0x00);
	snd_soc_component_write(codec, ES8316_SYS_VMIDSEL_REG0C, 0xFF);
	msleep(30);
	snd_soc_component_write(codec, ES8316_CLKMGR_CLKSEL_REG02, 0x08);
	snd_soc_component_write(codec, ES8316_CLKMGR_ADCOSR_REG03, 0x20);
	snd_soc_component_write(codec, ES8316_CLKMGR_ADCDIV1_REG04, 0x11);
	snd_soc_component_write(codec, ES8316_CLKMGR_ADCDIV2_REG05, 0x00);
	snd_soc_component_write(codec, ES8316_CLKMGR_DACDIV1_REG06, 0x11);
	snd_soc_component_write(codec, ES8316_CLKMGR_DACDIV2_REG07, 0x00);
	snd_soc_component_write(codec, ES8316_CLKMGR_CPDIV_REG08, 0x00);
	snd_soc_component_write(codec, ES8316_SDP_MS_BCKDIV_REG09, 0x04);
	snd_soc_component_write(codec, ES8316_CLKMGR_CLKSW_REG01, 0x7F);
	snd_soc_component_write(codec, ES8316_CAL_TYPE_REG1C, 0x0F);
	snd_soc_component_write(codec, ES8316_CAL_HPLIV_REG1E, 0x90);
	snd_soc_component_write(codec, ES8316_CAL_HPRIV_REG1F, 0x90);
	snd_soc_component_write(codec, ES8316_ADC_VOLUME_REG27, 0x00);
	snd_soc_component_write(codec, ES8316_ADC_PDN_LINSEL_REG22, 0xc0);
	snd_soc_component_write(codec, ES8316_ADC_D2SEPGA_REG24, 0x00);
	snd_soc_component_write(codec, ES8316_ADC_DMIC_REG25, 0x08);
	snd_soc_component_write(codec, ES8316_DAC_SET2_REG31, 0x20);
	snd_soc_component_write(codec, ES8316_DAC_SET3_REG32, 0x00);
	snd_soc_component_write(codec, ES8316_DAC_VOLL_REG33, 0x00);
	snd_soc_component_write(codec, ES8316_DAC_VOLR_REG34, 0x00);
	snd_soc_component_write(codec, ES8316_SDP_ADCFMT_REG0A, 0x00);
	snd_soc_component_write(codec, ES8316_SDP_DACFMT_REG0B, 0x00);
	snd_soc_component_write(codec, ES8316_SYS_VMIDLOW_REG10, 0x11);
	snd_soc_component_write(codec, ES8316_SYS_VSEL_REG11, 0xFC);
	snd_soc_component_write(codec, ES8316_SYS_REF_REG12, 0x28);
	snd_soc_component_write(codec, ES8316_SYS_LP1_REG0E, 0x04);
	snd_soc_component_write(codec, ES8316_SYS_LP2_REG0F, 0x0C);
	snd_soc_component_write(codec, ES8316_DAC_PDN_REG2F, 0x11);
	snd_soc_component_write(codec, ES8316_HPMIX_SEL_REG13, 0x00);
	snd_soc_component_write(codec, ES8316_HPMIX_SWITCH_REG14, 0x88);
	snd_soc_component_write(codec, ES8316_HPMIX_PDN_REG15, 0x00);
	snd_soc_component_write(codec, ES8316_HPMIX_VOL_REG16, 0xBB);
	snd_soc_component_write(codec, ES8316_CPHP_PDN2_REG1A, 0x10);
	snd_soc_component_write(codec, ES8316_CPHP_LDOCTL_REG1B, 0x30);
	snd_soc_component_write(codec, ES8316_CPHP_PDN1_REG19, 0x02);
	snd_soc_component_write(codec, ES8316_CPHP_ICAL_VOL_REG18, 0x00);
	snd_soc_component_write(codec, ES8316_GPIO_SEL_REG4D, 0x00);
	snd_soc_component_write(codec, ES8316_GPIO_DEBUNCE_INT_REG4E, 0x02);
	snd_soc_component_write(codec, ES8316_TESTMODE_REG50, 0xA0);
	snd_soc_component_write(codec, ES8316_TEST1_REG51, 0x00);
	snd_soc_component_write(codec, ES8316_TEST2_REG52, 0x00);
	snd_soc_component_write(codec, ES8316_SYS_PDN_REG0D, 0x00);
	snd_soc_component_write(codec, ES8316_RESET_REG00, 0xC0);
	msleep(50);
	snd_soc_component_write(codec, ES8316_ADC_PGAGAIN_REG23, 0x60);
	snd_soc_component_write(codec, ES8316_ADC_D2SEPGA_REG24, 0x01);
	/* adc ds mode, HPF enable */
	snd_soc_component_write(codec, ES8316_ADC_DMIC_REG25, 0x08);
	snd_soc_component_write(codec, ES8316_ADC_ALC1_REG29, 0xcd);
	snd_soc_component_write(codec, ES8316_ADC_ALC2_REG2A, 0x08);
	snd_soc_component_write(codec, ES8316_ADC_ALC3_REG2B, 0xa0);
	snd_soc_component_write(codec, ES8316_ADC_ALC4_REG2C, 0x05);
	snd_soc_component_write(codec, ES8316_ADC_ALC5_REG2D, 0x06);
	snd_soc_component_write(codec, ES8316_ADC_ALC6_REG2E, 0x61);
	return 0;
}
...
static int es8316_resume(struct snd_soc_component *codec)
{
	...
	ret = snd_soc_component_read(codec, ES8316_CLKMGR_ADCDIV2_REG05);
	if (!ret) {
		es8316_init_regs(codec);
		snd_soc_component_write(codec, ES8316_GPIO_SEL_REG4D, 0x00);
		/* max debance time, enable interrupt, low active */
		snd_soc_component_write(codec, ES8316_GPIO_DEBUNCE_INT_REG4E, 0xf3);
...
int es8316_headset_detect(int jack_insert)
{
	...
	if (jack_insert) {
		snd_soc_component_update_bits(es8316_codec,
...
static int es8316_probe(struct snd_soc_component *codec)
{
	...
	if (!ret) {
		...
		if (!ret) {
			...
			snd_soc_component_write(codec, ES8316_HPMIX_VOL_REG16, 0x00);
			if (!es8316->hp_inserted)
				snd_soc_component_write(codec, ES8316_SYS_PDN_REG0D,
					      0x3F);
			snd_soc_component_write(codec, ES8316_SYS_LP1_REG0E, 0xFF);
			snd_soc_component_write(codec, ES8316_SYS_LP2_REG0F, 0xFF);
			snd_soc_component_write(codec, ES8316_CLKMGR_CLKSW_REG01, 0xF3);
...
static void es8316_remove(struct snd_soc_component *codec)
{
	es8316_set_bias_level(codec, SND_SOC_BIAS_OFF);
}
...
static int es8316_i2c_probe(struct i2c_client *i2c)
{
	...
	es8316->spk_ctl_gpio = of_get_named_gpio(np,
						       "spk-con-gpio",
						       0);
	...
	es8316->hp_det_gpio = of_get_named_gpio(np,
						      "hp-det-gpio",
						      0);
